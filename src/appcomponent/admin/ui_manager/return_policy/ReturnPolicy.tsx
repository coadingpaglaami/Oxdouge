...
import { ReturnHelp } from "./ReturnHelp";
...
const PolicyCard = ({
  ...
  policy: ReturnPolicyItem;
  onEdit: (p: ReturnPolicyItem) => void;
  onDelete: (p: ReturnPolicyItem) => void;
}) => {
    ...
    <div className="bg-[#1C1C1C] border border-[#2A2A2A] rounded-xl px-6 py-5 flex flex-col gap-4 transition-colors duration-200 hover:border-[#FFD345] group">
      <div className="flex justify-between items-start gap-3">
        <div className="flex gap-3 items-center flex-1 min-w-0">
          <RotateCcw size={18} className="text-[#FFD345] shrink-0" />
...
interface PolicyFormDialogProps {
  ...
  initialData?: ReturnPolicyItem | null;
  isLoading: boolean;
  title: string;
}
...
export const ReturnPolicy = () => {
  ...
  const [editTarget, setEditTarget] = useState<ReturnPolicyItem | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<ReturnPolicyItem | null>(
    null,
  );

  const { data, isLoading, error } = useGetReturnPolicyQuery({
    page,
    limit: PAGE_SIZE,
  });

  const [addReturnPolicy, { isLoading: isAdding }] =
    useAddReturnPolicyMutation();
  const [updateReturnPolicy, { isLoading: isUpdating }] =
    useUpdateReturnPolicyMutation();
  const [deleteReturnPolicy, { isLoading: isDeleting }] =
    useDeleteReturnPolicyMutation();
  ...
  const handleAdd = async ({ heading, points }: PolicyFormData) => {
    try {
      await addReturnPolicy({
        heading,
        content: pointsToContent(points),
      }).unwrap();
      setIsAddOpen(false);
      toast.success("Return policy has been created successfully.");
    } catch {
      toast.error("Failed to add return policy. Please try again.");
    }
  };
  ...
  const handleUpdate = async ({ heading, points }: PolicyFormData) => {
    ...
    try {
      ...
      setEditTarget(null);
      toast.success("Return policy has been updated successfully.");
    } catch {
      toast.error("Failed to update return policy. Please try again.");
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteReturnPolicy({ id: deleteTarget.id }).unwrap();
      setDeleteTarget(null);
      toast.success("Return policy has been deleted.");
      if (data?.results.length === 1 && page > 1) setPage((p) => p - 1);
    } catch {
      toast.error("Failed to delete return policy. Please try again.");
    }
  };
    ...
    <div className="bg-[#121212] min-h-screen p-10 font-sans">
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16 text-red-400 bg-[#1C1C1C] rounded-xl border border-[#2A2A2A]">
          <p className="m-0">
            Failed to load return policies. Please try again.
          </p>
        </div>
      ) : data?.results.length === 0 ? (
        <div className="text-center py-16 text-[#555] bg-[#1C1C1C] rounded-xl border border-dashed border-[#2A2A2A]">
          <RotateCcw size={36} className="text-[#2A2A2A] mx-auto mb-3" />
          <p className="m-0 text-[15px]">
            No return policies yet. Add your first one!
          </p>
        </div>
      ) : (
        <div className="flex flex-col gap-3">
          {data?.results.map((policy: ReturnPolicyItem) => (
            <PolicyCard
              key={policy.id}
              policy={policy}
